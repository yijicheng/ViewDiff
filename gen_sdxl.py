...
def load_sd(pretrained_model_name_or_path: str = "stabilityai/stable-diffusion-xl-base-1.0", device: str = "cuda") -> StableDiffusionXLPipeline:
    pipe = StableDiffusionXLPipeline.from_pretrained(pretrained_model_name_or_path, torch_dtype=torch.float16).to(device)
...
def main(
    ...
    for category, sequence_dict in prompts.items():
        ...
        for s in tqdm(sequences, desc=f"Generate for {category}"):
            ...
            image_counter = 0
            for p in prompts:
                images = pipe(p, num_images_per_prompt=num_images_per_prompt, height=576, width=1024).images
                for img in images:
